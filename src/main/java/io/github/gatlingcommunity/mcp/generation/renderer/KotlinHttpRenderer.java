...
public final class KotlinHttpRenderer extends RendererSupport implements GatlingCodeRenderer {
    @Override
    ...
    public String render(TargetContext target, HttpSimulationPlan plan) {
        return """
                ...
                %s
                  ...
                  private fun scenarioBuilder() = run {
                %s
                    scenario("%s")%s
                  }
                ...
                steps(plan).stream().map(KotlinHttpRenderer::step).collect(Collectors.joining()),
    ...
    private static String step(HttpScenarioStepPlan step) {
        return switch (step.type()) {
            ...
            case "group" -> "\n      .group(\"%s\").on(%s)"
                    .formatted(escape(step.group().name()), nested(step.group().steps()));
            case "repeat", "during", "forever" -> "\n      ." + loop(step);
            case "ifEquals", "doIf" -> "\n      .doIf(%s).then(%s)"
                    .formatted(kotlinCondition(step.conditional()), nested(step.conditional().steps()));
    ...
    private static String loop(HttpScenarioStepPlan step) {
        if ("during".equals(step.type())) {
            return "during(Duration.ofSeconds(%d)).on(%s)"
                    .formatted(step.loop().durationSeconds(), nested(step.loop().steps()));
        }
    ...
    private static String check(CheckPlan check) {
        if ("status".equals(check.type())) {
            return "status().shouldBe(%s)".formatted(javaExpected(check));
        }
