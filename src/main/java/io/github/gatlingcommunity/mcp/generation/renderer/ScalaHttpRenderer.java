...
public final class ScalaHttpRenderer extends RendererSupport implements GatlingCodeRenderer {
    @Override
    ...
    public String render(TargetContext target, HttpSimulationPlan plan) {
        return """
                ...
                %s
                  ...
                  val scn = scenario("%s")%s

                  setUp(
                    scn.inject(
                      rampUsersPerSec(%d).to(%d).during(%d.seconds),
                      constantUsersPerSec(%d).during(%d.seconds)
                    )
                ...
                steps(plan).stream().map(ScalaHttpRenderer::step).collect(Collectors.joining()),
    ...
    private static String step(HttpScenarioStepPlan step) {
        return switch (step.type()) {
            ...
            case "pause" -> "\n    .pause(%d.seconds)".formatted(step.pause().durationSeconds());
            case "group" -> "\n    .group(\"%s\").on(%s)".formatted(escape(step.group().name()), nested(step.group().steps()));
            case "repeat", "during", "forever" -> "\n    ." + loop(step);
            case "ifEquals", "doIf" -> "\n    .doIf(%s)(%s)"
                    .formatted(scalaCondition(step.conditional()), nested(step.conditional().steps()));
    ...
    private static String loop(HttpScenarioStepPlan step) {
        if ("during".equals(step.type())) {
            return "during(%d.seconds)(%s)".formatted(step.loop().durationSeconds(), nested(step.loop().steps()));
        }
        if ("forever".equals(step.type())) {
            return "forever(%s)".formatted(nested(step.loop().steps()));
        }
        return "repeat(%d)(%s)".formatted(step.loop().count(), nested(step.loop().steps()));
    }
